 ...
 * 
 */
public class LaporanSenaraiPenghuniKuartersMengikutKelasBulanTahun extends LebahModule {
	...
	private String getTajukLaporan() {
		String tajukLaporan = "Laporan Senarai Penghuni Kuarters Mengikut Bulan Dan Tahun";
	...
	private String getSQL() {
		...
		sb.append("SELECT rujBulan.keterangan AS x, count(penghuni.id) as y FROM ruj_bulan rujBulan ");
		sb.append("LEFT JOIN ");
		sb.append("(select penghuni.id as id,month(penghuni.tarikh_masuk_kuarters)as bulan, ");
		sb.append("penghuni.id_kuarters as idKuarters from kua_penghuni penghuni ");
		sb.append("LEFT JOIN kua_kuarters kuarters ON penghuni.id_kuarters=kuarters.id ");
		...
		if (!"".equalsIgnoreCase(tahun)){
			sb.append("and year(penghuni.tarikh_masuk_kuarters) = '"+tahun+"') ");
		}else{
			sb.append("and year(penghuni.tarikh_masuk_kuarters) = '2016') ");	
		}
		
		sb.append("penghuni ON penghuni.bulan=rujBulan.id ");
		sb.append("GROUP BY rujBulan.id ORDER BY rujBulan.id + 0");
		return sb.toString();
	}

//	#SELECT rujBulan.keterangan AS x, count(penghuni.id) as y FROM ruj_bulan rujBulan
//	LEFT JOIN
//	(select penghuni.id as id,month(penghuni.tarikh_masuk_kuarters)as bulan,
//	penghuni.id_kuarters as idKuarters from kua_penghuni penghuni
//	LEFT JOIN kua_kuarters kuarters ON penghuni.id_kuarters=kuarters.id
//	where id_kuarters is not null and kuarters.id_kelas_kuarters='A'
//	and year(penghuni.tarikh_masuk_kuarters) = '2016') penghuni 
//	ON penghuni.bulan=rujBulan.id
//	GROUP BY rujBulan.id ORDER BY rujBulan.id + 0
	
	private String getPath() {
		return "bph/modules/eis/kuarters/laporanSenaraiPenghuniKuarters";
	}
