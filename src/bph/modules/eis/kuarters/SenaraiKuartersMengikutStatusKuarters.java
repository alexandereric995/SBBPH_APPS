 ...
 * 
 */
public class SenaraiKuartersMengikutStatusKuarters extends LebahModule {
	...
	private String getTajukLaporan() {
		String tajukLaporan = "Bilangan Kuarters Mengikut Status Kuarters";
	...
	private String getSQL() {
		...
		sql = "SELECT ruj_status_kuarters.keterangan AS x, count(kuarters.id_kuarters) AS y"
				+ " FROM    ruj_status_kuarters"
				+ " LEFT JOIN"
				+ " (SELECT kua_kuarters.id AS id_kuarters,"
				+ " kua_kuarters.id_lokasi_kuarters AS id_lokasi_kuarters,"
				+ " kua_kuarters.id_kelas_kuarters AS id_kelas_kuarters,"
				+ " kua_kuarters.id_status_kuarters AS id_status_kuarters"
				+ " FROM kua_kuarters) kuarters"
				+ " ON kuarters.id_status_kuarters = ruj_status_kuarters.id WHERE id_kuarters IS NOT NULL"; 
			
		//parameter
			if (!"".equals(getParam("findKelasKuarters"))) {
				sql = sql + " AND id_kelas_kuarters = '" + getParam("findKelasKuarters") + "'";
			}
			
			if (!"".equals(getParam("findLokasiKuarters"))) {
				sql = sql + " AND id_lokasi_kuarters = '" + getParam("findLokasiKuarters") + "'";
			}
			
			sql = sql +" GROUP BY ruj_status_kuarters.id ORDER BY ruj_status_kuarters.keterangan ASC";
			  
		return sql;
	}

	private String getPath() {
		return "bph/modules/eis/kuarters/senaraiKuartersMengikutStatusKuarters";
	}
