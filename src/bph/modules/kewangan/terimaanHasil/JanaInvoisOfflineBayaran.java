...
public class JanaInvoisOfflineBayaran extends LebahRecordTemplateModule<PembayarLain>{
	...
	public String getPath() {
		...
		return "bph/modules/kewangan/janaInvoisOfflineBayaran";
	}
...
//				kodMesin = listModPayment.get(h).getMesin().getKodMesin();
			...
			//resit.setPembayar(db.find(Users.class, payerId));
			
			//String noResit = UtilKewangan.generateReceiptNo(mp,juruwang);
			//String noResit = UtilKewangan.generateReceiptNo(mp,juruwang);
			String noResit = getParam("noResit");
...
//			}
	...
	@Command("getMaklumatPembayarBerdaftar")
	public String getMaklumatPembayarBerdaftar() throws Exception {
		String noPendaftaran = get("id").trim();
		try {
			mp = new MyPersistence();
			Users pembayar = (Users) mp.find(Users.class, noPendaftaran);
			if (pembayar != null) {
				context.put("pembayar", pembayar);
			} else {
				context.put("pembayar", "");
			}
			context.put("noPendaftaran", noPendaftaran);
		} catch (Exception e) {
			System.out.println("Error getMaklumatPembayarBerdaftar : "+ e.getMessage());
		} finally {
			if (mp != null) { mp.close(); }
		}
		return getPath() + "/daftarPembayar.vm";
	}
}
