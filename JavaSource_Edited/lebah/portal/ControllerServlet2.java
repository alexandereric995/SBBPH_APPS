...
/*    */ 
/*    */   
/* 24 */   public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException { doPost(req, res); }
/*    */ 
/*    */ 
/*    */ 
/*    */   
...
/* 35 */       this.engine = (VelocityEngine)session.getAttribute("VELOCITY_ENGINE");
/* 36 */       this.context = (VelocityContext)session.getAttribute("VELOCITY_CONTEXT");
/*    */       
...
/*    */       } 
/*    */     } 
/* 44 */     myLogger.debug("ControllerServlet2:User= " + session.getAttribute("_portal_login") + "/ Session Id = " + session.getId() + " | className=" + className);
/*    */ 
/*    */ 
/*    */ 
/*    */ 
/*    */ 
/*    */     
/* 51 */     String securityToken = (String)session.getAttribute("securityToken");
/*    */     
/* 53 */     Date now = new Date();
/*    */     
/* 55 */     myLogger.debug("[SECURITY TOKEN][" + now + "] " + securityToken);
/*    */     
...
/*    */     } else {
/* 79 */       System.out.println("[" + className + "] userAgent = " + userAgent);
/* 80 */     }  if (userAgent.indexOf("Windows") > 0) {
/* 81 */       PCDeviceController.doService(getServletContext(), getServletConfig(), this.engine, this.context, session, req, res);
/*    */     }
/* 83 */     else if (userAgent.indexOf("MIDP") > 0) {
/* 84 */       System.out.println("[" + className + "] userAgent = " + userAgent);
/*    */     }
/* 86 */     else if (userAgent.indexOf("MMP") > 0) {
/* 87 */       System.out.println("[" + className + "] userAgent = " + userAgent);
/*    */     } else {
/* 89 */       PCDeviceController.doService(getServletContext(), getServletConfig(), this.engine, this.context, session, req, res);
/*    */     } 
/*    */   }
/*    */ }


/* Location:              D:\EclipseWorkspace\SBBPH\sbbphv2\WebContent\WEB-INF\lib\lebah-xe.jar!\lebah\portal\ControllerServlet2.class
