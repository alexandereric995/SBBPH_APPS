...
/*     */{
	...
	/*     */public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if (!"bph.portal.FrmLupaKataLaluan".equals(req.getPathInfo().substring(
				1))) {
		...
		/* 45 */synchronized (this) {
			/* 46 */this.engine = (VelocityEngine) session
					.getAttribute("VELOCITY_ENGINE");
			/* 47 */this.context = (VelocityContext) session
			...
			/* 49 */if (this.engine == null || this.context == null) {
				...
				/*     */}
			/*     */}
		/* 55 */myLogger.debug("ControllerServlet3:User= "
				+ session.getAttribute("_portal_login") + "/ Session Id = "
				+ session.getId());
		/* 56 */this.context.put("util", new Util());
		/*     */
		/* 58 */String prev_token = (session.getAttribute("form_token") != null) ? (String) session
				.getAttribute("form_token") : "";
		/*     */
		/* 60 */String form_token = (req.getParameter("form_token") != null) ? req
				.getParameter("form_token") : "empty";
		/*     */
		...
		/* 85 */session.setAttribute("_portal_appname",
		...
		/* 95 */if (hasSecurityToken) {
			/*     */
			/* 97 */String securityTokenURI = pathInfo.substring(0,
					pathInfo.indexOf("/"));
			/*     */
			/* 99 */String securityToken = (String) session
					.getAttribute("securityToken");
			/* 100 */this.context.put("securityToken", securityToken);
			/*     */
			/* 102 */if (!securityTokenURI.equals(securityToken)) {
				/* 103 */securityTokenDenied(this.engine, this.context, req,
						res);
				/* 104 */allowed = false;
				/*     */}
			/*     */}
		...
		/* 109 */if (allowed) {
			...
			/* 117 */if ("127.0.0.1".equals(remoteAddr))
				localAccess = true;
			/* 118 */this.context.put("session", session);
			/* 119 */String ddir = (req.getPathInfo().lastIndexOf("/") == 0) ? "../"
					: "../../";
			/*     */
			/* 121 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/prototype.js\" ></script>");
			/* 122 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/scriptaculous.js\" ></script>");
			/* 123 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/fixed.js\" ></script>");
			/* 124 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/dragdrop.js\" ></script>");
			/* 125 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/unittest.js\" ></script>");
			/* 126 */out.println("<script type=\"text/javascript\" src=\""
					+ ddir + "scriptaculous/ajax.js\" ></script>");
			/*     */
			/* 128 */JS_CSS js_css = new JS_CSS(this.engine, this.context, req,
					res);
			/*     */try {
				/* 130 */js_css.print();
			...
			/*     */try {
				...
				/*     */try {
					...
					/* 142 */if (module.indexOf(".") > 0) {
						...
						/* 150 */content = ClassLoadManager.load(className,
					...
					/* 154 */if (content instanceof GenericPortlet) {
						...
						/* 158 */Hashtable portletState = getPortletState(
								getServletConfig(), req, res, out, portletInfo);
						/* 159 */RenderRequest renderRequest = (RenderRequest) portletState
								.get("renderRequest");
						/* 160 */RenderResponse renderResponse = (RenderResponse) portletState
								.get("renderResponse");
						/* 161 */PortletConfig config = (PortletConfig) portletState
								.get("config");
						/* 162 */GenericPortlet portlet = (GenericPortlet) content;
						/* 163 */portlet.init(config);
						/* 164 */portlet.render(renderRequest, renderResponse);
						/*     */} else {
						/* 166 */((VTemplate) content).setEnvironment(
								this.engine, this.context, req, res);
						/* 167 */((VTemplate) content)
								.setServletContext(getServletConfig()
										.getServletContext());
						/* 168 */((VTemplate) content)
								.setServletConfig(getServletConfig());
						/* 169 */((VTemplate) content).setDiv(false);
						/*     */
						/* 171 */if (content instanceof Attributable) {
							/* 172 */Hashtable h = UserPage
									.getValuesForAttributable(module);
							/* 173 */if (h != null) {
					...
					/*     */
					/* 190 */content = new ErrorMsg(this.engine, this.context,
							req, res);
					/* 191 */((ErrorMsg) content)
							.setError("ClassNotFoundException : "
									+ cnfex.getMessage());
					/* 192 */} catch (InstantiationException iex) {
					/* 193 */content = new ErrorMsg(this.engine, this.context,
							req, res);
					/* 194 */((ErrorMsg) content)
							.setError("InstantiationException : "
									+ iex.getMessage());
					/* 195 */} catch (IllegalAccessException illex) {
					/* 196 */content = new ErrorMsg(this.engine, this.context,
							req, res);
					/* 197 */((ErrorMsg) content)
							.setError("IllegalAccessException : "
									+ illex.getMessage());
					/* 198 */} catch (Exception ex) {
					/* 199 */content = new ErrorMsg(this.engine, this.context,
							req, res);
					/* 200 */((ErrorMsg) content)
	...
	/*     */
	/*     */private static void securityTokenDenied(VelocityEngine engine,
			VelocityContext context, HttpServletRequest req,
			HttpServletResponse res) {
		/*     */try {
			/* 253 */VTemplate content = new SecurityTokenDenied(engine,
					context, req, res);
			/* 254 */content.print();
 ...
 * !\lebah\portal\ControllerServlet3.class Java compiler version: 7 (51.0)
