 ...
 * 
 */
public class SenaraiKuartersDalamPenyelenggaraan extends LebahModule {
	...
	private String getSQL() {
		...
		String sql = "";
		sql = "SELECT (CASE"
		      +" WHEN id_status_kuarters = 01 THEN 'BERPENGHUNI'"
		      +" WHEN id_status_kuarters = 02 THEN 'PENYELENGGARAAN'"
		      +" WHEN id_status_kuarters = 03 THEN 'SEDIA DIDUDUKI'"
		      +" ELSE ''"
		      +" END)"
		      +" AS x,"
		      +" count(id_status_kuarters) AS y"
		      +" FROM kua_kuarters WHERE kua_kuarters.id_status_kuarters in('01','02','03')";
		...
		sql = sql +" GROUP BY id_status_kuarters";
		
		return sql;
	}

	private String getPath() {
		return "bph/modules/eis/kuarters/senaraiKuartersPenyelenggaraan";
	}
