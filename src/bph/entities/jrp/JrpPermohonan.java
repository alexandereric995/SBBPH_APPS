...
public class JrpPermohonan {
	...
	public void setTarikhPengesahanKetuaJabatan(
			Date tarikhPengesahanKetuaJabatan) {
	...
	public String getStatusPerjanjian() {
		...
		if (("1424860634493").equals(status.getId())
				|| ("1424860634499").equals(status.getId())) {
			...
			if (tarikhTamat != null && tarikhTamat.toString().length() > 0) {
				...
				int diffYear = calTamat.get(Calendar.YEAR)
						- calCurrent.get(Calendar.YEAR);
				int diffMonth = diffYear * 12 + calTamat.get(Calendar.MONTH)
	...
	public String getBilMesyuarat() {
		...
		if (("1424860634490").equals(status.getId())) {
			bil = (String) db
					.get("Select x.bilMesyuarat from JrpKertasPertimbangan x where x.jrpPermohonan.id = '"
							+ id + "'");
		}
	...
	public String getSewaLatest() throws Exception {
		...
		try {
			System.out
					.println("Select x from JrpPermohonanLokasi x where x.permohonan.id = '"
							+ id + "'");
			List<JrpPermohonanLokasi> pl = db
					.list("Select x from JrpPermohonanLokasi x where x.permohonan.id = '"
							+ id + "'");

			if (pl != null) {
				sewaLama = (Double) db
						.get("Select x.sewaSebulan from JrpPermohonanLokasi x where x.permohonan.id = '"
								+ id + "' and x.flagLokasi = 'S'"); // SEWA LAMA
				sewaBaru = (Double) db
						.get("Select x.sewaSebulan from JrpPermohonanLokasi x where x.permohonan.id = '"
								+ id + "' and x.flagLokasi = 'B'"); // SEWA BARU
