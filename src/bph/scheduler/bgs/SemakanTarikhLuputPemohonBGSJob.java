...
public class SemakanTarikhLuputPemohonBGSJob implements Job {
	
	static Logger myLogger = Logger.getLogger("SemakanTarikhLuputPemohonBGSJob");
	private MyPersistence mp;

	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		myLogger.info("Executing SemakanTarikhLuputPemohonBGSJob on : " + new Date());
		System.out.println("Executing SemakanTarikhLuputPemohonBGSJob on : " + new Date());
		...
		myLogger.info("Finish SemakanTarikhLuputPemohonBGSJob on : " + new Date());
		System.out.println("Finish SemakanTarikhLuputPemohonBGSJob on : " + new Date());
	}
		...
		try {
			...
			if (user != null) {
				...
				if (userRole != null) {
					...
					// DELETE MAIN USER
					if ("(BGS) Pemohon".equals(user.getRole().getName())) {
						mp.remove(user);
					}
				}
			}			
