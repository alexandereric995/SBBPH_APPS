...
public class RppPermohonan {
	...
	public String getCatatanKeluar(){
		...
		try {
			...
			RppPengurusanBilik b = (RppPengurusanBilik) mp.get("select a from RppPengurusanBilik a where a.permohonan.id = '"+this.id+"' ");
			if(b!=null){
	...
	public String getKeteranganLainValue() {
		...
		try {
			mp = new MyPersistence();
			RppKelompokSenaraiAktiviti ku = (RppKelompokSenaraiAktiviti) mp.get("select a from RppKelompokSenaraiAktiviti a where a.permohonan.id = '"+this.id+"' and a.aktiviti.id = '06' ");
			if(ku!=null){
				str = ku.getKeteranganAktivitiLain();
			}
		} catch (Exception e) {
			System.out.println("Error getKeteranganLainValue : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return str;
	...
	public String checkVIP(){
		...
		try {
			...
			if(this.pemohon!=null){
				obj = (RppPemohonVIP) mp.get("select x from RppPemohonVIP x where x.pemohon.id = '"+this.pemohon.getId()+"' and x.flagVip = 'Y' ");
			}
	...
	public String statusBayaranLewat(){
		...
		if(tarikhAkhirBayaran!=null){
			if(today.after(tarikhAkhirBayaran) || today.equals(tarikhAkhirBayaran)){
				status = "LEWAT";
			}
		}
		return status;
	...
	public String keteranganStatusBayaran(){
		String status = "";
		if(this.statusBayaran !=null && this.statusBayaran.equalsIgnoreCase("Y")){
			status = "TELAH BAYAR";
		}else{
			status = "BELUM BAYAR";
		}
	...
	public Double amaunBayaranBalikWithDeposit(){
		...
		if(days < 14){
			...
			if(amaun < 50){
				//amaun = 50.00; 
				amaun = 0.00; /**Burn. atas persetujuan mampu - IR masa testing mampu pd 04122015*/
			}
		}
	...
	public Double amaunTotalSewaRpWithoutDeposit(){
		...
		try {
			...
			if(this.id != null){
				ak = mp.list("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id <> '72311' ");
			}
	...
	public Double amaunTotalSewaRpWithDeposit(){
		...
		try {
			...
			List<RppAkaun> ak = null;
			if(this.id != null){
				ak = mp.list("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' ");
			}
	...
	public Double amaunDeposit(){
		...
		try {
			...
			RppAkaun ak = null;
			if(this.id != null){
				ak = (RppAkaun) mp.get("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id = '72311' ");
			}
			if(ak != null){
				amaun = ak.getDebit();
			}
	...
	public boolean reminderBayaranKelompok(){
		...
		if(this.tarikhMasukRpp!=null && !this.statusBayaran.equalsIgnoreCase("Y")){
	...
	public Integer jumlahBilanganKerosakan(){
		...
		try {
			mp = new MyPersistence();
			List<RppAduanKerosakan> list = mp.list("select x from RppAduanKerosakan x where x.permohonan.id = '"+this.id+"' ");
			for(int i=0;i<list.size();i++){
				bil = bil + list.get(i).getKuantiti();
			}
		} catch (Exception e) {
			System.out.println("Error jumlahBilanganKerosakan : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return bil;
	...
	public Double jumlahNilaiKerosakan(){
		...
		try {
			...
			System.out.println("Error jumlahNilaiKerosakan : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return jumlah;
	...
	public Integer getTarikhPermohonanInYear() {
		...
		if(this.tarikhPermohonan!=null){
			year = Integer.parseInt(Util.getDateTime(this.tarikhPermohonan,"yyyy"));
		}
		return year;
	}
	...
	public String getIdResitSewa() {
		...
		try {
			...
			if(this.resitSewa !=null){
				...
				List<RppAkaun> akInvois = mp.list("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id = '74299' ");
				
				if(akInvois.size() > 0){
					oneAk = akInvois.get(0);
					if(oneAk!=null){
						oneInv = (KewInvois) mp.get("select x from KewInvois x where x.idLejar = '"+oneAk.getId()+"' ");
						if(oneInv!=null){
							rsi = (KewResitSenaraiInvois) mp.get("select x from KewResitSenaraiInvois x where x.invois.id = '"+oneInv.getId()+"' ");
							if(rsi!=null){
	...
	public String getIdResitDeposit() {
		...
		try {
			...
			if(this.resitDeposit !=null){
				...
				KewResitSenaraiInvois rsi = null;
				List<RppAkaun> akInvois = mp.list("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id = '72311' ");
				
				if(akInvois.size() > 0){
					oneAk = akInvois.get(0);
					if(oneAk!=null){
						oneDep = (KewDeposit) mp.get("select x from KewDeposit x where x.idLejar = '"+oneAk.getId()+"' ");
						if(oneDep!=null){
							rsi = (KewResitSenaraiInvois) mp.get("select x from KewResitSenaraiInvois x where x.deposit.id = '"+oneDep.getId()+"' ");
							if(rsi!=null){
								str = rsi.getResit().getId();
							}
						}
	...
	public KewTuntutanDeposit getObjTuntutanDeposit() {
		...
		try {
			mp = new MyPersistence();
			if(this.id!=null){
				RppAkaun ak = (RppAkaun) mp.get("select w from RppAkaun w where w.permohonan.id = '"+this.id+"' and w.kodHasil.id = '72311' ");
				if(ak!=null){
					KewDeposit dep = (KewDeposit) mp.get("select x from KewDeposit x where x.idLejar = '"+ak.getId()+"' ");
					if(dep!=null){
						obj = (KewTuntutanDeposit) mp.get("select x from KewTuntutanDeposit x where x.deposit.id = '"+dep.getId()+"' ");
					}
				}
			}
		} catch (Exception e) {
			System.out.println("Error getObjTuntutanDeposit : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return obj;
	...
	public KewSubsidiari getObjKewSubsidiari() {
		...
		try {
			mp = new MyPersistence();
			if (this.id != null) {
				obj = (KewSubsidiari) mp.get("select x from KewSubsidiari x where x.idFail = '"+this.id+"' ");
			}			
		} catch (Exception e) {
...
//		try {
//			mp = new MyPersistence();
//			akaun = (RppAkaun) mp.get("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id='72311'");
//		} catch (Exception e) {
//			System.out.println("Error getObjKewSubsidiari : "+e.getMessage());
//		}finally{
//			if (mp != null) { mp.close(); }
...
//			mp = new MyPersistence();
//			akaun = (RppAkaun) mp.get("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id='72311'");
//			d= (KewDeposit) mp.get("select x from KewDeposit x where x.idLejar = '"+akaun.getId()+"' ");
//		} catch (Exception e) {
//			System.out.println("Error getObjKewSubsidiari : "+e.getMessage());
//		}finally{
//			if (mp != null) { mp.close(); }
...
//	}
	...
	public KewTuntutanDeposit getObjKewTuntutanDeposit() {
		...
		try {
			...
			akaun = (RppAkaun) mp.get("select x from RppAkaun x where x.permohonan.id = '"+this.id+"' and x.kodHasil.id='72311'");
			d= (KewDeposit) mp.get("select x from KewDeposit x where x.idLejar = '"+akaun.getId()+"' ");
			td = (KewTuntutanDeposit) mp.get("select x from KewTuntutanDeposit x where x.deposit.id = '"+d.getId()+"' ");
		} catch (Exception e) {
			System.out.println("Error getObjKewSubsidiari : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
	...
	public Integer getKelompokValue() {
		...
		if(this.id!=null){
			try {
				db1 = new Db();
				String sql = "SELECT x.bil_unit, y.kadar_sewa FROM rpp_kelompok_unit x, ruj_jenis_unit_rpp y WHERE x.id_jenis_unit_rpp = y.id AND x.id_permohonan = '"+this.id+"'  ";
				ResultSet rs = db1.getStatement().executeQuery(sql);	
				while (rs.next()) { 
					val += rs.getInt("bil_unit"); 
					//totalHarga += (rs.getDouble("kadar_sewa") * rs.getInt("bil_unit"));
				}	
			}catch(Exception e){
				System.out.println("Error getKelompokValue[Total] : "+e.getMessage());
			}finally { 
	...
	public Double getKelompokValueHarga() {
		...
		if(this.id!=null){
			try {
				...
				System.out.println("Error getKelompokValueHarga[Total] : "+e.getMessage());
			}finally { 
				if ( db1 != null ) db1.close();
			}
		}
	...
	public Integer getKelompokLulusValue() {
		...
		if(this.id!=null){
			try {
				db1 = new Db();
				String sql = "SELECT x.bil_unit_lulus FROM rpp_kelompok_unit x WHERE x.id_permohonan = '"+this.id+"'  ";
				ResultSet rs = db1.getStatement().executeQuery(sql);	
				while (rs.next()) { val += rs.getInt("bil_unit_lulus"); }	
			}catch(Exception e){
				System.out.println("Error getKelompokLulusValue[Total] : "+e.getMessage());
			}finally { 
				if ( db1 != null ) db1.close();
			}
		}
	...
	public Double getKelompokLulusValueHarga() {
		...
		if(this.id!=null){
			try {
				db1 = new Db();
				String sql = "SELECT x.bil_unit_lulus, y.kadar_sewa FROM rpp_kelompok_unit x, ruj_jenis_unit_rpp y WHERE x.id_jenis_unit_rpp = y.id AND x.id_permohonan = '"+this.id+"'  ";
				ResultSet rs = db1.getStatement().executeQuery(sql);	
				while (rs.next()) { 
					totalHarga += (rs.getDouble("kadar_sewa") * rs.getInt("bil_unit_lulus"));
				}	
			}catch(Exception e){
				System.out.println("Error getKelompokLulusValueHarga[Total] : "+e.getMessage());
			}finally { 
				if ( db1 != null ) db1.close();
			}
		}
