...
public class FrmKuaPermohonanOverallRecord extends FrmKuaPermohonanRecord {

	private static final long serialVersionUID = 8985571158382218466L;
	static Logger myLogger = Logger
			.getLogger("bph/modules/qtr/FrmKuaPermohonanOverallRecord");
	...
	public Map<String, Object> searchCriteria() throws Exception {
		...
		r.put("noPermohonan", getParam("findNoPermohonan").trim());
		r.put("pemohon.userName", getParam("findNamaPemohon").trim());
		r.put("pemohon.id", getParam("findNoKPPemohon").trim());
		r.put("lokasi.id", getParam("findLokasiPermohonan").trim());
		r.put("datePermohonan", new OperatorDateBetween(getDate("findTarikhPermohonan"),getDate("findTarikhPermohonan")));
		r.put("status.id", getParam("findStatus"));
		return r;
	}
	...
	public void getRelatedData(KuaPermohonan r) {
		...
		try {
			...
			context.put("uj", uj);
			getNoGiliranManual(r.getId());
			getNoGiliran(r.getId());
		} catch (Exception e) {
		...
		try {
			...
			if (pekerjaan != null) {
				...
				if (pekerjaan.getGredJawatan() != null) {
					...
					if (pekerjaan.getKelasPerkhidmatan() != null) {
				...
				if (kp.getLokasi().getId().equalsIgnoreCase("03")){
					if(kelasLayak.equalsIgnoreCase("A") || kelasLayak.equalsIgnoreCase("B") ||kelasLayak.equalsIgnoreCase("C") ||kelasLayak.equalsIgnoreCase("D")){
						kelasLayak = "E";
					}
					kelasDowngrade = "";
				}
			...
			//06082018 ADD BY PEJE - LIST CATATAN PERMOHONAN
			if (kp != null) {
				List <KuaCatatanPermohonan> listCatatanPermohonan = mp.list("select x from KuaCatatanPermohonan x where x.permohonan.id = '" + kp.getId() + "' order by x.tarikhDaftar asc");
				context.put("listCatatanPermohonan", listCatatanPermohonan);
			}
		} catch (Exception e) {
	...
	public String checkJarakLangitudeLotitude(MyPersistence mp) {
		...
		try {
			...
			// local----------------------------------------------
			// array =
			// KuartersUtils.getDistanceBetweenLocation(alamatRumah,mercuTanda);
	...
	public void simpanAuditTrail(String idPermohonan, String flagProses) {
		...
		Users u = (Users) mp.find(Users.class, kp.getPemohon().getId());
		UsersSpouse us = (UsersSpouse) mp.get("SELECT us FROM UsersSpouse us WHERE us.users.id = '"+ kp.getPemohon().getId() + "'");
	...
	@SuppressWarnings("unchecked")
	public String getNoGiliranManual(String idPermohonan) throws Exception {
		try {
			context.put("rekodManual", false);
			mp = new MyPersistence();
			KuaPermohonan kp = (KuaPermohonan) mp.find(KuaPermohonan.class,
					idPermohonan);
			String idPemohon = kp.getPemohon().getId();
			List<Giliran> giliran = mp
					.list("SELECT x FROM Giliran x WHERE x.noKP = '" + idPemohon
							+ "'");
			if (giliran.size() > 0) {
				if (giliran.get(0) != null) {
					context.put("giliranManual", giliran);
					context.put("rekodManual", true);
				} else {
					context.put("rekodManual", false);
				}
			}
	...
	@SuppressWarnings("unchecked")
	public String getNoGiliran(String idPermohonan) throws Exception {
		try {
			context.put("rekod", false);
			mp = new MyPersistence();
			KuaPermohonan kp = (KuaPermohonan) mp.find(KuaPermohonan.class,idPermohonan);
			//String idPemohon = kp.getPemohon().getId();
			String idPemohonan = kp.getId();
			List<KuaAgihan> giliran = mp
					.list("SELECT x FROM KuaAgihan x WHERE x.permohonan.id = '" + idPemohonan
							+ "' and x.status.id='1419601227590'");
			if (giliran.size() > 0) {
				if (giliran.get(0) != null) {
