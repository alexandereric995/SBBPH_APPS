...
public class TempahanRPRecordModule extends
	...
	public boolean saveMainRecordInSql(String permohonanId, String userIdMohon)
		...
		String generatedNoTempahan = UtilRpp.generateNoTempahanIndividuSQL(
				kodLokasi, null);
	...
	public String batalTempahan() throws Exception {
		...
		try {
			...
			mp.commit();
			RppMailer.get().notifikasiPembatalanTempahanOlehPemohon(
					p.getPemohon().getEmel(), p);
		} catch (Exception ex) {
	...
	public String paparPilihan() throws Exception {
		...
		try {
			...
			System.out.println("Error paparPilihan FPX RP : " + e.getMessage());
		} finally {
			if (mp != null) {
				mp.close();
			}
	...
	public String doChangeBankFPX() throws Exception {
		...
		try {
			...
			fpxUtil.registerFPXRequest(fpx_sellerId, fpx_sellerExId,
					...
					fpx_productDesc, "IR", mp);
		} catch (Exception e) {
			System.out.println("Error doChangeBankFPX FPX RP : "
					+ e.getMessage());
		} finally {
			if (mp != null) {
				mp.close();
			...
			if (jenisUnit != null) {
				list = mp
						.list("select x from RppSelenggara x where x.id in "
								...
								+ " order by x.tarikhMula asc"); // tarikh
																	// sysdate
																	// dan
																	// kedepan
																	// dan by
