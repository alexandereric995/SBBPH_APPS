...
public class PengesahanPenggunaRecordModule extends
		LebahRecordTemplateModule<Users> {
	...
	public void begin() {
		...
		context.put("selectGredPerkhidmatan",
				dataUtil.getListGredPerkhidmatan()); // add byzul
	...
	public String simpanPengesahan() {
		...
		try {
			mp = new MyPersistence();
			Users pengguna = (Users) mp.find(Users.class,
					getParam("idPengguna"));
			...
			if (pengguna != null) {
				...
				pengguna.setPengesah(db.find(Users.class, request.getSession()
	...
	private void generateEmel(Users pengguna, String emel, String flagKeputusan) {
		if (flagKeputusan.equals("Y")) {
			DaftarAkaunBaruMailer.get().kelulusanPengesahanPengguna(pengguna,
					emel);
		} else if (flagKeputusan.equals("T")) {
			DaftarAkaunBaruMailer.get().penolakanPengesahanPengguna(pengguna,
					emel);
		}
	}
