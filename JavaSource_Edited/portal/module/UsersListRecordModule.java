...
public class UsersListRecordModule extends LebahRecordTemplateModule<Users> {
	...
	private void defaultValue(MyPersistence mp, String idPengguna) {
		...
		context.put("selectKategoriPengguna",
				dataUtil.getListKategoriPenggunaOnline());
		...
		if (pengguna != null && pengguna.getProfilePhoto() != "") {
			...
			if (source.exists()) {
				...
				context.put("avatarPhotoName",
						"profilePhoto/no_photo_avatar.jpg");
			}
		} else {
			context.put("profilePhotoName", "profilePhoto/no_photo.jpg");
		...
		if (pengguna != null
				&& "(AWAM) Penjawat Awam".equals(pengguna.getRole().getName())) {
			IntHRMIS hrmis = (IntHRMIS) mp
					.get("select x from IntHRMIS x where x.noPengenalan = '"
							+ pengguna.getId() + "'");
			if (hrmis != null) {
				context.put("tarikhSemakan", util.getDateTime(
						hrmis.getTarikhTerima(), "dd-MM-yyyy hh:mm:ss a"));
			} else {
		...
		if (pengguna != null
				&& "(AWAM) Pesara".equals(pengguna.getRole().getName())) {
			IntPESARA pesara = (IntPESARA) mp
					.get("select x from IntPESARA x where x.noPengenalan = '"
							+ pengguna.getId() + "'");
			if (pesara != null) {
				context.put("tarikhSemakan", util.getDateTime(
						pesara.getTarikhTerima(), "dd-MM-yyyy hh:mm:ss a"));
			} else {
				context.put("tarikhSemakan", "-");
			}
		}
	...
	public String kemaskiniPengguna() {
		...
		try {
			...
			System.out.println("ERROR KEMASKINI PROFIL PENGGUNA : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	public String uploadProfilPhoto() throws Exception {
		...
		for (FileItem item : files) {
			...
			if (type.equals("image")) {
				...
				lebah.repository.Thumbnail.create(
						ResourceBundle.getBundle("dbconnection").getString(
								"folder")
								+ imgName,
						ResourceBundle.getBundle("dbconnection").getString(
								"folder")
	...
	private void updateProfilePhoto(String idPengguna, String profilePhoto,
			String avatarName) {
		...
		try {
			...
			System.out.println("ERROR UPDATE PROFILE PHOTO : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	public String resetPassword() {
		...
		try {
			...
			if (pengguna != null) {
				mp.begin();
				if (pengguna.getEmel() == null
						|| pengguna.getEmel().length() == 0) {
					pengguna.setEmel(getParam("emel"));
				}
				pengguna.setUserPassword(PasswordService.encrypt(pengguna
						.getId()));
				mp.commit();
			...
			System.out.println("ERROR RESET PASSWORD PENGGUNA : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	private void generateEmel(Users user, String password, String emel) {
		if (user.getRole().getName().equals("(AWAM) Pengguna Awam")) {
			...
			if (user.getFlagAktif().equals("Y")) {
				...
				TetapanSemulaKataLaluanMailer.get()
						.hantarPasswordBadanBerkanun(emel, user.getId(),
								password);
			}
		} else {
			TetapanSemulaKataLaluanMailer.get().hantarPassword(emel,
					...
					if (pengguna != null) {
						if (hrmis != null) {
							...
							if (hrmis.getKodGelaran() != null)
								pengguna.setGelaran((Gelaran) mp.find(
										Gelaran.class, hrmis.getKodGelaran()));
							if (hrmis.getNoTelefonPejabat() != null)
								pengguna.setNoTelefonPejabat(hrmis
										.getNoTelefonPejabat());
							if (hrmis.getKodAgensi() != null)
								pengguna.setAgensi((Agensi) mp.find(
										Agensi.class, hrmis.getKodAgensi()));
							if (hrmis.getJabatan() != null)
								pengguna.setBahagian(hrmis.getJabatan());
							if (hrmis.getKodKelasPerkhidmatan() != null)
								pengguna.setKelasPerkhidmatan((KelasPerkhidmatan) mp
										.find(KelasPerkhidmatan.class,
												hrmis.getKodKelasPerkhidmatan()));
							if (hrmis.getKodGredPerkhidmatan() != null)
								pengguna.setGredPerkhidmatan((GredPerkhidmatan) mp
										.find(GredPerkhidmatan.class,
												hrmis.getKodGredPerkhidmatan()));
							if (hrmis.getKodEtnik() != null)
								pengguna.setEtnik((Etnik) mp.find(Etnik.class,
										hrmis.getKodEtnik()));
							if (hrmis.getKodStatusPerkahwinan() != null)
								pengguna.setStatusPerkahwinan((StatusPerkahwinan) mp
										.find(StatusPerkahwinan.class,
												hrmis.getKodStatusPerkahwinan()));
							pengguna.setFlagAktif("Y");
							pengguna.setFlagSemakanHRMIS("Y");

							// USER JOB
							boolean addJob = false;
							UsersJob job = (UsersJob) mp
									.get("select x from UsersJob x where x.users.id = '"
											+ pengguna.getId() + "'");
							if (job == null) {
								addJob = true;
								job = new UsersJob();
							...
							job.setJenisPerkhidmatan(pengguna
									.getJenisPerkhidmatan());
							job.setKelasPerkhidmatan(pengguna
									.getKelasPerkhidmatan());
							job.setAgensi(pengguna.getAgensi());
							job.setBahagian(pengguna.getBahagian());
							job.setBadanBerkanun(pengguna.getBadanBerkanun());
							if (hrmis != null) {
								job.setTarikhLantikan(hrmis
										.getTarikhMulaSandang());
								job.setTarikhTamat(hrmis
										.getTarikhTamatSandang());
								...
								if (hrmis.getKodBandarPejabat() != null)
									job.setBandar((Bandar) mp.find(
											Bandar.class,
											hrmis.getKodBandarPejabat()));
							...
							UsersSpouse spouse = (UsersSpouse) mp
							...
							if (hrmis != null) {
								...
								spouse.setJenisPengenalan((JenisPengenalan) mp
										.find(JenisPengenalan.class, "B"));
								spouse.setNoKPPasangan(hrmis
										.getNoPengenalanPasangan());
								spouse.setNoTelBimbit(hrmis
										.getNoTelefonPasangan());
								spouse.setJenisPekerjaan(hrmis
										.getPekerjaanPasangan());
								spouse.setNamaSyarikat(hrmis
										.getMajikanPasangan());
							}
							if (addSpouse)
								mp.persist(spouse);
					...
					if (pesara != null) {
						if (pesara != null) {
							...
							if (pesara.getKodJawatan() != null)
								pengguna.setJawatan((Jawatan) mp.find(
										Jawatan.class, pesara.getKodJawatan()));
							if (pesara.getJawatanTerakhir() != null)
								pengguna.setKeteranganJawatan(pesara
										.getJawatanTerakhir());
							if (pesara.getKelasPerkhidmatan() != null)
								pengguna.setKelasPerkhidmatan((KelasPerkhidmatan) mp
										.find(KelasPerkhidmatan.class,
												pesara.getKelasPerkhidmatan()));
							if (pesara.getGredPerkhidmatan() != null)
								pengguna.setGredPerkhidmatan((GredPerkhidmatan) mp
										.find(GredPerkhidmatan.class,
												pesara.getGredPerkhidmatan()));
							pengguna.setFlagAktif("Y");
	...
	public String uploadDokumenPengesahan() throws Exception {
		...
		for (FileItem item : files) {
			...
			String imgName = uploadDir + idPengguna
					+ fileName.substring(fileName.lastIndexOf("."));
	...
	private void updateDokumenPengesahan(String idPengguna,
			String dokumenPengesahan) {
		...
		try {
			...
			System.out.println("ERROR UPDATE DOKUMEN PENGESAHAN : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	public String refreshDokumenPengesahan() throws Exception {
		context.put("updateDokumenPengesahan",
				getParam("updateDokumenPengesahanReload"));
		context.remove("updatePerjawatanStatus");
		context.remove("semakanPerjawatanStatus");
		return getMaklumatPerjawatan();
	}
	...
	public String hapusDokumenPengesahan() {
		...
		try {
			...
			System.out.println("ERROR KEMASKINI DOKUMEN PENGESAHAN : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	public String kemaskiniPerjawatan() {
		...
		try {
			...
			if (!"".equals(getParam("idKelasPerkhidmatan"))) {
				kelasPerkhidmatan = (KelasPerkhidmatan) mp.find(
			...
			if (!"".equals(getParam("idGredPerkhidmatan"))) {
				gredPerkhidmatan = (GredPerkhidmatan) mp.find(
						GredPerkhidmatan.class, getParam("idGredPerkhidmatan"));
			}
	...
	public String kemaskiniMaklumatBank() {
		...
		try {
			...
			System.out.println("ERROR KEMASKINI MAKLUMAT BANK : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	private void updateLampiranAkaun(String idPengguna, String dokumenBank) {
		...
		try {
			...
			System.out.println("ERROR UPDATE LAMPIRAN AKAUN : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
	...
	public String refreshLampiranAkaun() throws Exception {
		context.put("updateLampiranAkaun",
				getParam("updateLampiranAkaunReload"));
		context.remove("updateMaklumatBank");
		return getDokumenSokongan();
	}
	...
	public String kemaskiniFlagGelanggang() {
		...
		try {
			...
			System.out.println("ERROR KEMASKINI FLAG GELANGGANG : "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			if (mp != null) {
