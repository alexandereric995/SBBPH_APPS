...
public class JenisUnitRPP {
	...
	//@OneToMany(cascade=CascadeType.ALL, mappedBy="jenisUnitRpp", fetch=FetchType.EAGER)
...
//	}
	...
	public Integer getTotalBilanganUnit() {
		...
		try {
			...
			if(this.id != null){
				val =  mp.list("select x from RppUnit x where x.jenisUnit.id = '"+this.id+"' ").size();
			}
		} catch (Exception e) {
			System.out.println("Error getTotalBilanganUnit : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return val;
	...
	public String getKeteranganGredMinimumKelayakan() {
		...
		try {
			...
			if(this.gredMinimumKelayakan != 0 ){
				...
				GredPerkhidmatan obj = (GredPerkhidmatan) mp.find(GredPerkhidmatan.class, id);
				if(obj !=null){
					str = obj.getKeterangan();
				}
			}
		} catch (Exception e) {
			System.out.println("Error getKeteranganGredMinimumKelayakan : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return str;
	...
	public String getKeteranganGredKelayakanWaktuPuncak() {
		...
		try {
			...
			System.out.println("Error getKeteranganGredKelayakanWaktuPuncak : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		return str;
	...
	public int getBilAvailabilityKelompok(String idPermohonan){
		...
		try {
			...
			if(idPermohonan != null && !idPermohonan.equalsIgnoreCase("")){
				...
				dtIn = new SimpleDateFormat("yyyy-MM-dd").format(r.getTarikhMasukRpp());
				dtOut = new SimpleDateFormat("yyyy-MM-dd").format(r.getTarikhKeluarRpp());
			}
		
		} catch (Exception e) {
			System.out.println("Error getBilAvailabilityKelompok[get r] : "+e.getMessage());
		}finally{
			if (mp != null) { mp.close(); }
		}
		...
		if(r!=null){
			...
			try {
				...
				sql = "select * from rpp_unit a "+
					  ...
					  " ON a.id = b.id_unit "+ 
					  " and ((tarikh_mula <= '"+dtIn+"' AND tarikh_tamat > '"+dtIn+"') "+ 
					  " OR (tarikh_mula < '"+dtOut+"' AND tarikh_tamat >= '"+dtOut+"') "+ 
					  " OR (tarikh_mula >= '"+dtIn+"' AND tarikh_tamat < '"+dtOut+"')) "+ 
					  " where a.id_jenis_unit = '"+this.id+"' "+ 
				...
				sql +=" and a.id not in (select c.id_unit from rpp_selenggara_unit_lokasi c, rpp_selenggara d "+
						...
						" and ((d.tarikh_mula <= '"+dtIn+"' AND d.tarikh_tamat > '"+dtIn+"') "+ 
						" OR (d.tarikh_mula < '"+dtOut+"' AND d.tarikh_tamat >= '"+dtOut+"') "+ 
						" OR (d.tarikh_mula >= '"+dtIn+"' AND d.tarikh_tamat < '"+dtOut+"'))) "+
						" and b.id is null ";
				...
				System.out.println("error getBilAvailabilityKelompok : "+e.getMessage());
				e.printStackTrace();
			}finally { 
	...
	public Integer getKelompokValue(String idPermohonan) {
		...
		if(idPermohonan!=null && this.id!=null){
			try {
				db1 = new Db();
				String sql = "SELECT x.bil_unit FROM rpp_kelompok_unit x WHERE x.id_permohonan = '"+idPermohonan+"' AND x.id_jenis_unit_rpp = '"+this.id+"' ";
				ResultSet rs = db1.getStatement().executeQuery(sql);	
				if (rs.next()) { val = rs.getInt("bil_unit"); }	
			}catch(Exception e){
				System.out.println("Error getKelompokValue() : "+e.getMessage());
			}finally { 
				if ( db1 != null ) db1.close();
			}
		}
	...
	public Integer getKelompokLulusValue(String idPermohonan) {
		...
		if(idPermohonan!=null && this.id!=null){
			try {
				db1 = new Db();
				String sql = "SELECT x.bil_unit_lulus FROM rpp_kelompok_unit x WHERE x.id_permohonan = '"+idPermohonan+"' AND x.id_jenis_unit_rpp = '"+this.id+"' ";
				ResultSet rs = db1.getStatement().executeQuery(sql);	
				if (rs.next()) { val = rs.getInt("bil_unit_lulus"); }	
			}catch(Exception e){
				System.out.println("Error getKelompokLulusValue() : "+e.getMessage());
			}finally { 
				if ( db1 != null ) db1.close();
			}
		}
	...
	public List<RppUnit> listRppUnitSale() {
		...
		try {
			mp = new MyPersistence();
			list = mp.list("select x from RppUnit x where x.jenisUnit.id = '"+this.id+"' and COALESCE(x.status,'') <> 'RESERVED' ");
		} catch (Exception e) {
