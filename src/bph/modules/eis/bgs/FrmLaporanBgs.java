...
public class FrmLaporanBgs extends LebahModule {

	/**
	 * 
	 */
	static Logger myLogger = Logger.getLogger(FrmLaporanBgs.class);
	...
	private String getPath() {
		return "bph/modules/eis/bgs/eisLaporanBgs";
	...
	public String getSQL_laporan1(String dateStart,String dateEnd) {
		...
		sb.append("select month(bgs_permohonan.tarikh_permohonan) AS X, ");
		sb.append("count(bgs_permohonan.id) AS Y ");
		sb.append("from bgs_permohonan ");
		if (dateStart != "" && dateEnd != "") {
			sb.append(" WHERE tarikh_permohonan BETWEEN str_to_date('"+dateStart+"','%Y') AND str_to_date('"+dateEnd+"','%Y')");
		}
		sb.append("group by month(bgs_permohonan.tarikh_permohonan), year(bgs_permohonan.tarikh_permohonan)");
		
		System.out.println("SQL getXML=> " + sb.toString());
		return sb.toString();
	}
	
	private String getSQL_laporan2(String dateStart, String dateEnd) {
		// TODO Auto-generated method stub
		StringBuffer sb = new StringBuffer();

		sb.append("select b.keterangan AS X, ");
		sb.append("count(a.id) AS Y ");
		sb.append("from bgs_permohonan a ");
		sb.append("join ruj_status b ");
		sb.append("on a.id_status = b.id ");
		if (dateStart != "" && dateEnd != "") {
