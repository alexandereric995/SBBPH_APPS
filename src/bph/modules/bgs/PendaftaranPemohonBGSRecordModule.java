...
public class PendaftaranPemohonBGSRecordModule extends LebahRecordTemplateModule<BgsPemohon> {
	...
	@Override
	public Class<BgsPemohon> getPersistenceClass() {
		// TODO Auto-generated method stub
		return BgsPemohon.class;
	}

	@Override
	public String getPath() {
		// TODO Auto-generated method stub
		return "bph/modules/bgs/pendaftaranPemohon";
	}
	...
	@Override
	public void save(BgsPemohon r) throws Exception {
		userId = (String) request.getSession().getAttribute("_portal_login");
		userRole = (String) request.getSession().getAttribute("_portal_role");
	...
	@Override
	public void afterSave(BgsPemohon r) {
		Users user = null;
		...
		try {
			...
			if (user == null) {
				...
				user.setRole((Role) mp.find(Role.class, "(BGS) Pemohon"));
				user.setFlagAktif("Y");
				...
				// GENERATE EMEL
				DaftarAkaunBaruMailer.get().hantarPasswordPemohonBGSJRP(r.getEmel(), r.getId(), c);
			} else {
				...
				if ("Y".equals(user.getFlagDaftarSBBPH())) {
					...
					UserRole userRole = (UserRole) mp.get("select x from UserRole x where x.userId = '" + r.getId() + "' and x.roleId = '(BGS) Pemohon'");
					if (userRole == null) {
						userRole = new UserRole();
						userRole.setUserId(r.getId());
						userRole.setRoleId("(BGS) Pemohon");
						mp.persist(userRole);		
					}					
					
					// GENERATE EMEL
					DaftarAkaunBaruMailer.get().hantarSuccessRolePemohonBGSJRP(r.getEmel(), r.getId(), "BGS");
	...
	@Override
	public boolean delete(BgsPemohon r) throws Exception {
		try {
			mp = new MyPersistence();
			...
			if (users != null) {
				...
				if (userRole != null) {
					...
					// DELETE MAIN USER
					if ("(BGS) Pemohon".equals(users.getRole().getName())) {
						mp.remove(users);
					}
				}
	...
	@Override
	public void getRelatedData(BgsPemohon r) {
		dataUtil = DataUtil.getInstance(db);
		if (r.getAgensi() != null) {
	...
	public String kemaskiniPemohon() throws Exception {
		...
		try {
			...
			pemohon = (BgsPemohon) mp.find(BgsPemohon.class, idPemohon);
			if (pemohon != null) {
				...
				if ("Y".equals(flagAktif)) {
					if ("T".equals(flagAktifDB)) {
						...
						if (users == null) {
							...
							users.setUserPassword(PasswordService.encrypt(c));
							...
							users.setRole((Role) mp.find(Role.class, "(BGS) Pemohon"));
							...
							// GENERATE EMEL
							DaftarAkaunBaruMailer.get().hantarPasswordPemohonBGSJRP(pemohon.getEmel(), pemohon.getId(), c);
						} else {
							// USER TELAH BERDAFTAR DENGAN SBBPH
							if ("Y".equals(users.getFlagDaftarSBBPH())) {
								...
								UserRole userRole = (UserRole) mp.get("select x from UserRole x where x.userId = '" + users.getId() + "' and x.roleId = '(BGS) Pemohon'");
								if (userRole == null) {
									userRole = new UserRole();
					...
					if ("Y".equals(flagAktifDB)) {
						//DELETE/DEACTIVATE USERS
						if (users != null) {
							//DELETE SECONDARY ROLE
							UserRole userRole = (UserRole) mp.get("select x from UserRole x where x.userId = '" + pemohon.getId() + "' and x.roleId = '(BGS) Pemohon'");
							if (userRole != null) {
								mp.remove(userRole);
							} else {
								// DELETE MAIN USER
